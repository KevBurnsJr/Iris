...
(function(){
  ...
  var reset = function() {
    ...
    shape = ['l', 'c'].indexOf(shape) >= 0 ? shape : 'l';
    ...
    for(i=depth_start; i > 0; i--) {
      var c = HSLtoRGB([(i/depth_start), .8, .5]);
      rgbl[i] = "rgba("+parseInt(c[0])+","+parseInt(c[1])+","+parseInt(c[2])+",.4)";
    }
  ...
  var drawFractal = function() {
    ...
    var lines = 0;
    var length = elem.height/(zoom*n);
    for(i = 0; i < n; i++) {
      next[depth_start].push([w/2, h/2, i*(360/n) + (rotation/n), 360 / n]);
    }
    if (shape == 'l') {
      for(i = depth_start; i > 0; i--) {
        ...
        for(j in next[i]) {
          ...
          if(depth > 1) {
            next[depth - 1].push([x2, y2, angle - (dangle + dcurl) + twist, dangle]);
            next[depth - 1].push([x2, y2, angle + (dangle + dcurl) + twist, dangle]);
          }
        }
        ctx.closePath();
    ...
    if (shape == 'c') {
      for(i = depth_start; i > 0; i--) {
        ...
        for(j in next[i]) {
          ...
          var x2 = x1 + (Math.cos(angle * deg_to_rad) * depth * length);
          var y2 = y1 + (Math.sin(angle * deg_to_rad) * depth * length);
          var a1 = (angle - (dangle + dcurl) + twist);
          var a2 = (angle + (dangle + dcurl) + twist);
          if(depth<depth_start) {
            ctx.beginPath();
            ctx.arc(x2, y2, i * length, 0, 360);
            ctx.stroke();
            lines++;
          }
          if(depth>1) {
            next[depth - 1].push([x2, y2, a1, dangle]);
            next[depth - 1].push([x2, y2, a2, dangle]);
          }
        }
      }
    }
  ...
  $('#control').on('change', 'input,select', function(el){
    $(el.target).unfocus();
    sethash();
  });
